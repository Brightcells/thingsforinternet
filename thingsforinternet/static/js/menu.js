...
var Menu = function (element, options) {
	...
	options = {degrees:options.degrees || 90, offset:options.degrees || -90, radius:options.radius || 150};
	this.node = element;
	...
	this.eachItem(function (item, index, items) {
		var degree = ((degrees / (items.length - 1)) * index) + offset, position = Menu.degreeToPosition(degree), x = Math.round((position.x * radius)), y = Math.round((position.y * radius)), name = "ani-" + index, delay = index * 6;
		var rules = [[0, "{-webkit-transform: translate3d(0px, 0px, 0px); -webkit-transition-timing-function: ease-out; }"], [(delay + 15), "{ -webkit-transform: translate3d(0px, 0px, 0px);}"], [(45 + index * 3), "{ -webkit-transform: translate3d(" + (x * 1.2) + "px, " + (y * 1.2) + "px, 0px); -webkit-transition-timing-function: ease-in; }"], [65, "{ -webkit-transform: translate3d(" + (x * 0.95) + "px, " + (y * 0.95) + "px, 0px); -webkit-transition-timing-function:ease-in-out}"], [100, "{ -webkit-transform: translate3d(" + x + "px, " + y + "px, 0px);}"]];
		var open = "", reverse = "";
...
Menu.prototype.open = function () {
	...
	this.eachAnimation(function (animation) {
		var item = animation[0], open = animation[1], link = item.querySelector("a");
